...
import { signupSchema, loginSchema, insertChitFundSchema } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/chit-funds", authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.get("/api/chit-funds/:id", authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      const fund = await storage.getChitFund(req.params.id);
      if (!fund) return res.status(404).json({ message: "Chit fund not found" });

      const members = await storage.getMembershipsByChitFund(fund.id);
      const userMembership = await storage.getMembership(req.userId!, fund.id);
  ...
  app.post("/api/chit-funds/:id/join", authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      const existing = await storage.getMembership(req.userId!, req.params.id);
      if (existing) {
        return res.status(409).json({ message: "Already joined this chit fund" });
      }

      const membership = await storage.createMembership(req.userId!, req.params.id);
  ...
  app.get("/api/my-memberships", authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.get("/api/my-payments", authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.get("/api/my-upcoming-payments", authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.get("/api/dashboard", authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const totalReturns = membershipsData.reduce((sum, m) => sum + m.chitFund.totalAmount, 0);

      return res.json({
        user: { id: user.id, name: user.name, email: user.email, phone: user.phone, role: user.role },
        activeFunds: membershipsData.filter((m) => m.chitFund.status === "active").length,
  ...
  // Admin routes
  app.post("/api/admin/chit-funds", authMiddleware, adminMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      ...
      const fund = await storage.createChitFund({ ...parsed.data, createdBy: req.userId! });
  ...
  app.put("/api/admin/chit-funds/:id", authMiddleware, adminMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      const updated = await storage.updateChitFund(req.params.id, req.body);
      if (!updated) return res.status(404).json({ message: "Chit fund not found" });
  ...
  app.delete("/api/admin/chit-funds/:id", authMiddleware, adminMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      const deleted = await storage.deleteChitFund(req.params.id);
      if (!deleted) return res.status(404).json({ message: "Chit fund not found" });
  ...
  app.get("/api/admin/users", authMiddleware, adminMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      ...
      return res.json(allUsers.map((u) => ({
        id: u.id, name: u.name, email: u.email, phone: u.phone, role: u.role, createdAt: u.createdAt,
      })));
    } catch (error: any) {
      return res.status(500).json({ message: error.message });
    }
  ...
  app.post("/api/admin/assign-user", authMiddleware, adminMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      const { userId, chitFundId } = req.body;
      if (!userId || !chitFundId) {
        return res.status(400).json({ message: "userId and chitFundId are required" });
      }
  ...
  app.post("/api/admin/create-payments", authMiddleware, adminMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      const { chitFundId } = req.body;
      if (!chitFundId) return res.status(400).json({ message: "chitFundId required" });
  ...
  app.put("/api/admin/payments/:id/status", authMiddleware, adminMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      const { status } = req.body;
      const paidDate = status === "paid" ? new Date() : undefined;
      const updated = await storage.updatePaymentStatus(req.params.id, status, paidDate);
  ...
  app.get("/api/admin/chit-funds/:id/members", authMiddleware, adminMiddleware, async (req: AuthRequest, res: Response) => {
    try {
  ...
  app.post("/api/admin/seed", async (req: Request, res: Response) => {
    try {
      ...
      await storage.createChitFund({
        ...
        description: "Premium monthly chit fund for high-value investments with guaranteed returns.",
        totalAmount: 500000,
      ...
      await storage.createChitFund({
        ...
        description: "Affordable monthly chit fund ideal for beginners looking to save regularly.",
        totalAmount: 100000,
      ...
      await storage.createChitFund({
        ...
        description: "High-yield chit fund with shorter duration for quick returns.",
        totalAmount: 200000,
